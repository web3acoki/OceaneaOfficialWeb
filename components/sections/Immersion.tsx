export default function Immersion() {
  const cardTitleClassName =
    "ft-[32/1320] font-medium leading-normal tracking-[-0.96px] text-white text-center";
  const cardDescClassName =
    "ft-[24/1320] font-normal leading-[25px] tracking-[-0.72px] text-[rgba(255,255,255,0.6)] text-center";
  ...
  return <>
    <section className="fmx-[300/1920] fmt-[430/1920] aspect-1320/1200 @container-[size]">
      <div className="relative w-1120/1320 fmx-[100/1320] aspect-1120/150 flex flex-col items-center justify-center">
        ...
        <p className={"absolute ft-[96/1320] font-medium ftr-[-2.88/1320] text-center"}>Enter The Oceanverse</p>
        <img src="/immersion-right.svg" className="absolute bottom-0 right-0 w-45/1120"/>
      </div>
      <p className={"fmt-[18/1920] ft-[28/1320] tracking-[-0.84px] text-[#a9a9a9] whitespace-pre-line text-center"}>{subtitleText}</p>
      <div className="fmt-[136/1320] flex w-full justify-between">
        {immersionCards.map((card) => (
          <div key={card.title} className="relative aspect-314/741 w-314/1320 overflow-hidden rounded-[3.788cqw]">
            <img src={card.src} alt="" className={`${card.imageClassName} z-0`} />
            <div className="absolute bottom-[1.6%] left-[3.7%] right-[3.7%] h-[31.6%] rounded-[3.788cqw] bg-[rgba(255,255,255,0.1)] flex flex-col items-center z-10">
              <p className={"fmt-[35/300] ft-[32/1320] tracking-[-0.96px] text-white text-center"}>{card.title}</p>
              <p className={`fmt-[10/300] fmx-[35/300] ft-[24/1320] ftr-[-0.72/1320] flh-[25/1320] text-[rgba(255,255,255,0.6)] text-center`}>{card.desc}</p>
            </div>
          </div>
        ))}
      </div>
